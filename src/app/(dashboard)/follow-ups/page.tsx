  ...
  return (
    <>
      <PageHeader
        ...
        title="Follow-ups"
      />
      <form className="mt-6 grid gap-3 rounded-lg border border-border bg-surface p-4 sm:grid-cols-4"><label className="sr-only" htmlFor="follow-up-user">Responsável</label><select className="h-10 min-w-0 rounded-sm border border-input bg-surface px-3" defaultValue={params.assignedUserId ?? ""} id="follow-up-user" name="assignedUserId"><option value="">Todos responsáveis</option>{users.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select><label className="sr-only" htmlFor="follow-up-date">Data</label><input className="h-10 min-w-0 rounded-sm border border-input bg-surface px-3" defaultValue={params.date ?? ""} id="follow-up-date" name="date" type="date"/><label className="sr-only" htmlFor="follow-up-status">Status</label><select className="h-10 min-w-0 rounded-sm border border-input bg-surface px-3" defaultValue={params.status ?? "PENDING"} id="follow-up-status" name="status"><option value="PENDING">Pendentes</option><option value="COMPLETED">Concluídos</option><option value="CANCELED">Cancelados</option></select><button className="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground" type="submit">Filtrar</button></form>
